...
class Symato:
	...
	def is_sym(self, token_id): global MAXXX; return 276 <= token_id and token_id <= MAXXX # âm tiết không dấu thanh
	def vocab_size(self): global MAXXX; return MAXXX
	...
	def tids_to_utf8(self, tids):
		...
		while i < n:
			...
			if self.is_capitalized(tid):
				...
				if tid < 256: str.append(tid)
				else: str += bytes(token,"utf8")
				prev_tid = tid
	...
	def decode(self, tid):
		if isinstance(tid, int):
			...
			return [ self.itos[i] if i > 255 else chr(i) for i in tid ]
	...
	def encode(self, a, maxx=None, rev=False):
		...
		return compact_tids + [16] # 16 là <|end-of-text|> token

	def encode_(self, a, maxx=None, rev=False):
		global need_to_add_syms
			...
			if c == 16:
				...
				if idx > -1:
					try: sym_id = self.stoi[s[0:idx]]
						...
						sss = s[0:idx]
						if sss not in need_to_add_syms:
							need_to_add_syms.add(sss)
							print(sss, end=" ", flush=True)
...
assert tids == [276, 256, 274, 276, 258, 32, 32, 275, 276, 257, 16]
# print(tids, symato.tids_to_utf8(tids))
assert symato.tids_to_utf8(tids) == "a À  Á"
tids = symato.encode(text, rev=True)
assert tids == [275, 276, 257, 32, 32, 274, 276, 258, 276, 256, 16]
assert symato.tids_to_utf8(tids) == "Á  À a"

def decode_encode(x): 
	tids = symato.encode(x)#; print(tids)
	return symato.tids_to_utf8(tids)
tids = symato.encode("điều")
assert bytes(tids).decode() == "điều"
assert decode_encode("điều")=="điều"
